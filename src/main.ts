...
async function checkForUpdates() {
  ...
  if (!update.updateAvailable || !update.version || !update.downloadUrl) {
    ...
    return "Biz Suite Cloud POS is up to date.";
  }
  ...
  updateHelpEl.textContent = update.notes || "A new Biz Suite Cloud POS installer is available. Your existing settings will be retained.";
  downloadUpdateButtonEl.classList.remove("hidden");
  return `Biz Suite Cloud POS v${update.version} is available.`;
}
